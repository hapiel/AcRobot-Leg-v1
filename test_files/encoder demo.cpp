...
// https://github.com/f2knpw/Wiper_Motor_Servo
// https://hackaday.io/project/187675-strong-servo-motor-with-a-wiper-motor/details
...
*/
#define RPWM_PIN  5  // PWM output pin ; connect to IBT-2 pin 1 (RPWM)
#define LPWM_PIN  18   // PWM output pin ; connect to IBT-2 pin 2 (LPWM)
...
void setup()
{
  ...
  nbRot =  -55 ;                 //initialize nbRot to what you want
  targetRot2 = getMotorPos();     //initialize where we exactly are (nbRot + decimal part)
  targetRot = 55.0 ;             //initialize the target number of rotations of the motor shaft (0 at startup)
...
void loop()
{
  ...
  if (abs(prevRawValue - rawValue) > 15) timeOut = millis();        //rearm timeout if motor is running
